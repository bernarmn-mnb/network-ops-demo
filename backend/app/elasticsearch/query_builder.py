...
def build_filter_clauses(filters: dict) -> list:
    ...
    for field, value in filters.items():
        ...
        if field.endswith('_min'):
            ...
            clauses.append({
                'range': {actual_field: {'gte': value}}
            })
        elif field.endswith('_max'):
            # Range filter: max
            actual_field = field[:-4]
            clauses.append({
                'range': {actual_field: {'lte': value}}
            })
    ...
    if isinstance(obj, str):
        ...
        for item in obj:
            ...
            if isinstance(substituted, dict):
                non_comment_keys = [k for k in substituted.keys() if not k.startswith('_')]
...
class QueryBuilder:
    ...
    def build(
        ...
        if user_preferences:
            user_prefs.update({k: v for k, v in user_preferences.items() if v is not None})
        ...
        if session_context:
            session_ctx.update({k: v for k, v in session_context.items() if v is not None})
        ...
        has_personalization = (
            user_prefs.get('preferred_brands') or 
            user_prefs.get('preferred_categories') or
            session_ctx.get('recent_categories')
        )
    ...
    def _build_sort(self, sort_by: Optional[str], sort_dir: str) -> list:
        """Build sort clause."""
    ...
    def extract_fields(properties: dict, prefix: str = ''):
        for field_name, field_def in properties.items():
            ...
            fields[full_name] = {
                ...
                'aggregatable': field_type in ['keyword', 'integer', 'long', 'float', 'double', 'date', 'boolean'],
