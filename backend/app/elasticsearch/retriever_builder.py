...
class RetrieverBuilder:
    ...
    def build(
        ...
        use_rrf: bool = False,
        knn_config: Optional[dict] = None,
        semantic_config: Optional[dict] = None,
        reranker_config: Optional[dict] = None,
        collapse_config: Optional[dict] = None,
        ltr_config: Optional[dict] = None,
        mmr_config: Optional[dict] = None,
        ...
        if not retrievers:
            ...
            personalization_clauses, exclusion_clauses = self._build_personalization_clauses(
            ...
            if should_clauses or must_not_clauses:
                ...
                active_features = []
                if feature_weights:
                    active_features.append(f"rank_features: {list(feature_weights.keys())}")
                if user_preferences:
                    if user_preferences.get('preferred_brands'):
                        active_features.append(f"brands: {user_preferences['preferred_brands']}")
                    if user_preferences.get('preferred_categories'):
                        active_features.append(f"categories: {user_preferences['preferred_categories']}")
                if session_context and session_context.get('recent_categories'):
                    active_features.append(f"recent: {session_context['recent_categories']}")
                    
                logger.info(f"Browse mode with: {', '.join(active_features) if active_features else 'no scoring'}")
            else:
                ...
                logger.info("Browse mode with no scoring signals (all scores will be 1.0)")
            
            # Clean up None filter
            if "filter" in retrievers[0].get("standard", {}) and retrievers[0]["standard"]["filter"] is None:
        ...
        if len(retrievers) > 1:
            ...
            if use_linear:
                ...
                if hybrid_config:
                    ...
                    text_weight = hybrid_config.get('text_weight', 1.0 - semantic_weight)
                ...
                logger.info(f"Using Linear retriever: text={text_weight:.2f}, semantic={semantic_weight:.2f}")
            else:
        ...
        if mmr_config and mmr_config.get('enabled'):
            inner_retriever = self._wrap_with_diversify_retriever(inner_retriever, mmr_config, query)
            logger.info(f"MMR diversify enabled: lambda={mmr_config.get('lambda_', 0.5)}, field={mmr_config.get('field', 'embedding')}")
        ...
        if ltr_config and query and query.strip():
            ...
            logger.info(f"LTR rescore enabled: model={ltr_config.get('model_id')}, window={ltr_config.get('window_size')}")
    ...
    def _build_rank_feature_clauses(self, weights: dict) -> list:
        ...
        for feature, weight in weights.items():
            ...
            if not field:
                logger.debug(f"Skipping unknown rank_feature: {feature} (not in rankFeatureFields config)")
                continue
    ...
    def _build_personalization_clauses(
        ...
        if user_preferences:
            ...
            if preferred_brands and brand_field:
                ...
                logger.debug(f"Personalization: boosting {brand_field}={preferred_brands} by {brand_boost}x")
            elif preferred_brands and not brand_field:
                logger.debug(f"Skipping preferred_brands: no 'brand' in personalizationFields config")
            ...
            if preferred_categories and category_field:
                ...
                logger.debug(f"Personalization: boosting {category_field}={preferred_categories} by {category_boost}x")
            elif preferred_categories and not category_field:
                logger.debug(f"Skipping preferred_categories: no 'category' in personalizationFields config")
            ...
            if excluded_brands and brand_field:
                ...
                logger.debug(f"Personalization: excluding {brand_field}={excluded_brands}")
            elif excluded_brands and not brand_field:
                logger.debug(f"Skipping excluded_brands: no 'brand' in personalizationFields config")
        ...
        if session_context:
            ...
            if recent_categories and category_field:
                ...
                logger.debug(f"Session: boosting {category_field}={recent_categories} by {recent_boost}x")
            elif recent_categories and not category_field:
                logger.debug(f"Skipping recent_categories: no 'category' in personalizationFields config")
    ...
    def _wrap_with_reranker(self, retriever: dict, config: dict) -> dict:
        ...
        if not field:
            raise ValueError("Reranker config requires 'field' - the text field to rerank on")
        if not model_id:
            raise ValueError("Reranker config requires 'model_id' - the inference endpoint ID")
    ...
    def _build_collapse(self, config: dict) -> dict:
        ...
        if not collapse_field:
            raise ValueError("Collapse config requires 'field' - the field to group results by")
    ...
    def _wrap_with_diversify_retriever(
        ...
        if not field:
            raise ValueError("MMR config requires 'field' - the dense vector field name")
        if not inference_id:
            raise ValueError("MMR config requires 'inference_id' - the embedding model ID")
    ...
    def _wrap_with_ltr_rescorer(self, retriever: dict, config: dict, query: str) -> dict:
        """
        ...
        model_id = config.get('model_id')
        if not model_id:
            raise ValueError("LTR config requires 'model_id' - the trained LTR model ID")
