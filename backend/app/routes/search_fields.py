    ...
    for field in fields:
        if field.searchable and field.type == "text":
            ...
            if field.likely_purpose == "title":
                ...
                boost = 1.5
            config["searchFields"].append(
                f"{field.name}^{boost}" if boost != 1 else field.name
            )
...
async def get_fields() -> FieldsResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get index mapping: {e!s}"
        )
...
async def fields_health() -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Elasticsearch unavailable: {e!s}")
