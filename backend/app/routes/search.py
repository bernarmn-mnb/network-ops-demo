...
from ..elasticsearch.query_builder import QueryBuilder, get_index_fields, build_filter_clauses
from ..elasticsearch.retriever_builder import RetrieverBuilder
from ..elasticsearch.models import FeatureWeights, UserPreferences, SessionContext, FieldBoost, QueryTypeConfig
...
class LtrConfig(BaseModel):
    ...
    model_id: str = Field(
        default="demo-ltr-model", 
        description="Trained LTR model ID in Elasticsearch"
    )
...
class SearchRequest(BaseModel):
    ...
    feature_weights: Optional[FeatureWeights] = Field(default=None, description="Rank feature weights for lab mode")
    user_preferences: Optional[UserPreferences] = Field(default=None, description="User personalization preferences")
    session_context: Optional[SessionContext] = Field(default=None, description="Session context for ranking")
    explain: bool = Field(default=False, description="Include ranking explanation (matched_queries with scores)")
    query_type_config: Optional[QueryTypeConfig] = Field(default=None, description="Query type and field boosting config")
    debug: bool = Field(default=False, description="Include raw Elasticsearch query in response for debugging")
    use_retrievers: bool = Field(default=True, description="Use Retriever API instead of query DSL (ES 8.17+)")
    diversify: Optional[DiversifyConfig] = Field(default=None, description="Result diversification via field collapse")
    ltr: Optional[LtrConfig] = Field(default=None, description="Learning to Rank rescoring configuration")
...
class SearchConfigRequest(BaseModel):
    ...
    templateVars: Optional[dict] = None
    searchFields: Optional[list[str]] = None
    facets: Optional[list[dict]] = None
    rangeFilters: Optional[list[dict]] = None
...
class SearchHit(BaseModel):
    ...
    matched_queries: Optional[dict[str, float]] = None  # Named query -> score contribution
    inner_hits: Optional[list[InnerHit]] = None  # Variants when using field collapse/diversify
...
class SearchResponse(BaseModel):
    ...
    debug_query: Optional[dict] = Field(default=None, description="Raw Elasticsearch query (when debug=true)")
    diversify: Optional[DiversifyInfo] = Field(default=None, description="Diversification info when enabled")
...
def _build_hybrid_config(query_type_config: Optional[QueryTypeConfig]) -> Optional[dict]:
    """
...
async def search(request: SearchRequest) -> SearchResponse:
    ...
    with optional_span("search") as span:
        ...
        if request.sort_by:
            span and span.set_attribute("search.sort", f"{request.sort_by}:{request.sort_dir}")
        
        # Set feature weights if present (for ranking lab mode)
        if request.feature_weights:
            span and span.set_attribute("search.feature_weights", json.dumps(request.feature_weights.model_dump(exclude_none=True)))
        ...
        try:
            ...
            if request.feature_weights:
                feature_weights_dict = {
                    k: v for k, v in request.feature_weights.model_dump().items()
                    if v is not None
                }
            ...
            if request.user_preferences:
                user_preferences_dict = {
                    k: v for k, v in request.user_preferences.model_dump().items()
                    if v is not None
                }
                span and span.set_attribute("search.user_preferences", json.dumps(user_preferences_dict))
            
            # Convert session_context to dict if provided
            session_context_dict = None
            if request.session_context:
                session_context_dict = {
                    k: v for k, v in request.session_context.model_dump().items()
                    if v is not None
                }
                span and span.set_attribute("search.session_context", json.dumps(session_context_dict))
            ...
            if request.use_retrievers:
                ...
                filter_clauses = build_filter_clauses(request.filters) if request.filters else []
                ...
                if request.query.strip():
                    try:
                        ...
                        logger.info(f"Applying query rules via rule retriever: {MERCHANDISING_RULESET_ID}")
                    except Exception:
                ...
                if request.query_type_config and request.query.strip():
                    if request.query_type_config.query_type in ('semantic', 'hybrid'):
                        semantic_config = {
                            'field': request.query_type_config.semantic_field or 'semantic_text',
                            'query': request.query,
                        }
                ...
                if user_preferences_dict:
                    logger.info(f"Personalization: user_preferences={user_preferences_dict}")
                if session_context_dict:
                    logger.info(f"Personalization: session_context={session_context_dict}")
                ...
                if request.diversify and request.diversify.enabled:
                    ...
                    span and span.set_attribute("search.diversify_strategy", request.diversify.strategy)
                    ...
                    if request.diversify.strategy == 'mmr':
                        ...
                        span and span.set_attribute("search.mmr_lambda", request.diversify.mmr_lambda)
                        logger.info(f"MMR diversification enabled: lambda={request.diversify.mmr_lambda}, field={request.diversify.mmr_field}")
                    else:
                        ...
                        span and span.set_attribute("search.diversify_field", request.diversify.collapse_field)
                        logger.info(f"Field collapse enabled: field={request.diversify.collapse_field}")
                ...
                if request.ltr and request.ltr.enabled:
                    ...
                    span and span.set_attribute("search.ltr_model", request.ltr.model_id)
                    logger.info(f"LTR enabled: model={request.ltr.model_id}, window={request.ltr.window_size}")
            ...
            # =====================================================================
                ...
                if request.query_type_config:
                    ...
                    span and span.set_attribute("search.query_type", request.query_type_config.query_type)
                    ...
                    if request.query_type_config.query_type == 'semantic':
                        ...
                        config_to_use['templateVars'] = {
                            'semantic_field': request.query_type_config.semantic_field or 'semantic_text',
                        }
                        logger.info(f"Using semantic search template")
                        ...
                        semantic_weight = request.query_type_config.semantic_weight or 50
                        ...
                        logger.info(f"Using hybrid search template (text_boost={text_boost}, semantic_boost={semantic_boost})")
                ...
                if feature_weights_dict:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not check for rank_features: {mapping_err}")
                        rank_features_available = False
                    ...
                    if rank_features_available:
                        ...
                        if user_preferences_dict or session_context_dict:
                            ...
                            logger.info(f"Personalized Mode: Using personalized_named template")
                        else:
                            config_to_use['queryTemplate'] = 'rank_features'
                        logger.info(f"Lab Mode: Using {config_to_use['queryTemplate']} template with weights: {feature_weights_dict}")
                    else:
                        ...
                        feature_weights_dict = None  # Disable weights for simple template
                ...
                logger.info(f"Searching '{request.query}' in {index} (explain={request.explain})")
                ...
                if request.query.strip() and template_name in compatible_templates:
                    try:
                        ...
                        apply_rules = True
                        span and span.set_attribute("search.query_rules_applied", True)
                        logger.info(f"Applying query rules from ruleset: {MERCHANDISING_RULESET_ID}")
                    except Exception as ruleset_err:
                        # Ruleset doesn't exist or error - continue without rules
                        if "resource_not_found" not in str(ruleset_err).lower() and "404" not in str(ruleset_err):
                            logger.warning(f"Error checking query rules: {ruleset_err}")
                        span and span.set_attribute("search.query_rules_applied", False)
                elif request.query.strip() and template_name not in compatible_templates:
                    span and span.set_attribute("search.query_rules_applied", False)
                    logger.info(f"Skipping query rules - template '{template_name}' not compatible with rule_query")
            ...
            span and span.set_attribute("search.result_ids", result_ids[:10])  # Top 10 IDs
...
async def update_config(request: SearchConfigRequest) -> dict:
    ...
    logger.info(f"Config updated: index={_search_config['index']}, template={_search_config['queryTemplate']}")
...
class Suggestion(BaseModel):
    ...
    text: str
    score: Optional[float] = None
    source: Optional[dict] = None  # Product data if from product index
...
async def suggest(
    ...
    with optional_span("search.suggest") as span:
        ...
        try:
            ...
            logger.info(f"Suggest: query='{q}' results={len(suggestions)} took={took_ms}ms")
...
async def get_capabilities() -> IndexCapabilities:
    ...
    try:
        ...
        has_semantic = len(semantic_fields) > 0  # Only true if semantic_text fields exist
        ...
        if semantic_fields:
            messages.append(f"Found {len(semantic_fields)} semantic_text field(s): {', '.join(semantic_fields)}")
        if dense_vector_fields:
            messages.append(f"Found {len(dense_vector_fields)} dense_vector field(s): {', '.join(dense_vector_fields)}")
        if not semantic_fields and not dense_vector_fields:
            messages.append("No semantic/vector fields found. Hybrid/Semantic search unavailable.")
        ...
        return IndexCapabilities(
            ...
            semantic_fields=[],
            searchable_fields=[],
            message=f"Failed to check capabilities: {str(e)}"
        )
