...
async def track_click(event: ClickEvent):
    ...
    try:
        ...
        with tracer.start_as_current_span("search.result.click") as span:
            logger.info(
                f"Span created: {type(span).__name__}, name={span.name if hasattr(span, 'name') else 'N/A'}"
            )
            ...
            if event.product_category:
                span.set_attribute(
                    "search.result_click_category", event.product_category
                )
        ...
        return TrackingResponse(
            status="success", message=f"Click recorded for document {event.document_id}"
        )

    except Exception as e:
        logger.error(f"Failed to track click: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to track click: {e!s}")
...
async def track_cart(event: CartEvent):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to track cart: {e!s}")
...
async def track_checkout(event: CheckoutEvent):
    ...
    try:
        ...
        return TrackingResponse(
            status="success", message=f"Checkout recorded for order {event.order_id}"
        )

    except Exception as e:
        logger.error(f"Failed to track checkout: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to track checkout: {e!s}")
