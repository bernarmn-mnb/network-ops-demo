...
_monitoring_client: Optional[Elasticsearch] = None
...
def get_monitoring_client() -> Elasticsearch:
    ...
    logger.info(f"Connecting to monitoring cluster: {settings.MONITORING_ELASTICSEARCH_URL}")
...
TEMPLATES_DIR = Path(__file__).parent.parent / "elasticsearch" / "templates" / "analytics"
...
class QueryJudgmentDocument(BaseModel):
    ...
    doc_id: str
    title: Optional[str] = None  # Enriched from products index
...
async def execute_esql(query: str) -> dict:
    ...
    try:
        ...
        raise NoDataError("Monitoring cluster not configured. Check MONITORING_ELASTICSEARCH_URL in .env")
        ...
        if "Unknown index" in error_str or "index_not_found" in error_str:
            ...
            raise NoDataError("No search analytics data available. Use the Search page to generate data.")
...
async def analytics_health():
    ...
    try:
        ...
        return HealthResponse(
            ...
            has_data=False,
            trace_count=0,
            message=f"Failed to query analytics: {str(e)}",
            monitoring_configured=True,
            otel_configured=otel_configured,
...
-- CTR = (clicks / searches) * 100"""
    ...
    try:
        ...
        # Calculate CTR
        ctr = round((total_clicks / total_searches * 100), 2) if total_searches > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch CTR: {str(e)}")
...
-- Rate = (zero_results / total) * 100"""
    ...
    try:
        ...
        try:
            ...
            zero_result_searches = zero_rows[0].get("zero_result_searches", 0) if zero_rows else 0
        except NoDataError:
            zero_result_searches = 0
        
        # Calculate rate
        zero_results_rate = round(zero_result_searches * 100.0 / total_searches, 2) if total_searches > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch zero results rate: {str(e)}")


@router.get("/top-queries", response_model=TopQueriesResponse)
async def get_top_queries(
    range: TimeRange = Query(default="24h", description="Time range for analytics"),
    limit: int = Query(default=20, ge=1, le=100, description="Number of queries to return")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch top queries: {str(e)}")


@router.get("/click-distribution", response_model=ClickDistributionResponse)
async def get_click_distribution(
    range: TimeRange = Query(default="24h", description="Time range for analytics"),
    limit: int = Query(default=10, ge=1, le=20, description="Number of positions to return")
):
    """
    ...
    try:
        ...
        distribution = [
            ClickPosition(
                ...
                percentage=round(row.get("clicks", 0) * 100.0 / total_clicks, 2) if total_clicks > 0 else 0.0
            )
            for row in rows
        ]
...
async def get_zero_result_queries(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch zero result queries: {str(e)}")
...
async def get_query_judgments(
    ...
    range: TimeRange = Query(default="24h", description="Time range for analytics"),
    enrich: bool = Query(default=True, description="Enrich with product titles from index")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch query judgments: {str(e)}")


async def enrich_with_product_data(documents: list[QueryJudgmentDocument]) -> list[QueryJudgmentDocument]:
    """
