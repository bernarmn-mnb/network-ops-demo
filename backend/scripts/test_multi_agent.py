...
def test_check_agents() -> TestResult:
    ...
    try:
        ...
        for agent in agents:
            ...
            agent_id = agent.get("url", "").split("/a2a/")[-1] if "/a2a/" in agent.get("url", "") else "unknown"
...
def test_single_agent_call() -> TestResult:
    ...
    try:
        ...
        if passed:
            print(f"✅ Single agent call successful")
        else:
            print(f"⚠️  No agent was called")
...
def test_routing_decision() -> TestResult:
    ...
    message = "What information do you have available? Can you help me search for something?"
    print(f"📤 Message: {message}")
    ...
    try:
        ...
        if passed:
            ...
            print(f"⚠️  No agent routing occurred")
...
def test_complex_query() -> TestResult:
    ...
    try:
        ...
        if passed:
            ...
            print(f"⚠️  Complex query may not have been fully processed")
...
def test_client_function_handling() -> TestResult:
    ...
    try:
        ...
        if len(client_calls) > 0:
            print(f"✅ Client function was called!")
        else:
            print(f"ℹ️  LLM responded without calling client function")
...
def test_custom_system_prompt() -> TestResult:
    ...
    try:
        ...
        if passed:
            print(f"✅ Custom system prompt handled")
