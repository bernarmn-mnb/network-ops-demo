  ...
  // =========================================================================
  {
    ...
    scenarios: [
      ...
      },
      {
        ...
        badge: '2',
        badgeColor: 'success',
        title: 'Meraki Analysis — Clickable Drill-through to Kibana',
        keyInsight: 'Every KPI, bar chart row, and device row is a live link — click to open Kibana Discover pre-filtered to that exact value.',
        steps: [
          'Navigate to Meraki Analysis (/meraki)',
          'Show KPI row — note the "↗ Open in Kibana" label under each card',
          'Click "Air Marshal Events" KPI → Kibana Discover opens filtered to airmarshal_events',
          'Click "Security Alerts" KPI → filtered to ids_alerted / security_event events',
          'Back on the page: click a domain bar row (e.g. Comcast CDN) → Discover filtered to that url.domain',
          'Click a Source IP bar row → Discover filtered to that client IP showing all their events',
          'Click a Device bar row → Discover showing all events from that Meraki AP or MX',
          'Scroll to Device Inventory — click any device row → Discover filtered to that device',
          'Point out amber rows for outdated firmware — these are real vulnerabilities',
          'Show Air Marshal events feed if populated — rogue APs on your network',
        ],
        talkingPoints: [
          '"Every number on this page is a doorway — click it and Kibana opens pre-filtered to exactly that data"',
          '"108 Air Marshal events — click that KPI and you\'re looking at every rogue AP detection, timestamped and device-attributed"',
          '"Click a source IP and you see that client\'s full browsing history, every domain they hit, every security event they triggered"',
          '"The device inventory row click takes you straight to all events from that physical device — MX68, Tiffany_AP, whatever you click"',
          '"Outdated firmware rows are highlighted amber — this is your real attack surface, not a mock-up"',
          '"The cisco data view (covers *cisco*) unifies all Meraki logs and NetFlow in one query surface"',
        ],
        demoPills: [
